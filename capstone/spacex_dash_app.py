...
spacex_df = pd.read_csv('/Users/liyifei/Desktop/vscode/coursea/capstone/spacex_launch_dash.csv')
max_payload = spacex_df['Payload Mass (kg)'].max()
min_payload = spacex_df['Payload Mass (kg)'].min()
steps = (max_payload - min_payload)/10
...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        fig = px.pie(filtered_df, names='Launch Site', title='Total Success Launches by Sites')
    else:
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]
        class_counts = filtered_df['class'].value_counts().reset_index()
        class_counts.columns = ['class', 'count']
        colors = ['red' if c == 0 else 'green' for c in class_counts['class']]
